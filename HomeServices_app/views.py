...
class bookservice(LoginRequiredMixin, View):
    ...
    def post(self, request, id):
        ...
        return redirect('bookservice', id=id)
...
class feedback_form(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        return redirect('feedback_form')  # Make sure this name matches the one in urls.py
...
def edit_profile(request):
    try:
        user = users.objects.get(admin=request.user)
    except users.DoesNotExist:
        messages.error(request, "User does not exist.")
        return redirect('userprofile')  # or wherever you want to go
    ...
    return render(request, 'edit_profile.html', {'data': user})
...
def activate_worker(request, id):
    ...
    worker.acc_activation = 1
    worker.save()
    return redirect('manageworker')

def deactivate_worker(request, id):
    worker = get_object_or_404(workers, id=id)
    worker.acc_activation = 0
    worker.save()
    return redirect('manageworker')
